...
int main() {
  ...
  // Must be called otherwise it will cause the display not to function
  // normally.
  ...
  // // Times by 4 so that 1 quater becomes 1 beat ima forget what i mean by
  // this but oh well long whole_note = ((60 * 1000 * 1000) * 4) / tempo;
  ...
  //     int beat_loop_count = ((whole_note / duration) /
  //     (frequency_pause_duration << 1)); beat_loop_count *= beat_loop_count <
  //     0 ? -1.5 : 1;
  ...
  while (true) {
    if (gpio_irq_flags.set_time) {
      ...
      show = !show;  // Toggle mode
      gpio_irq_flags.set_confirm = 0;
      // Reset the static variables upon mode change.
      if (show) {
        reset_clock_static_vars();
        set_char(2, ':', false);
        set_char(5, ':', false);
        update_clock();  // Force immediate update
      } else {
        set_char(2, '/', false);
        set_char(5, '/', false);
      }
