 ...
 */
char *
append_chars (const char *original, char character_to_append, size_t num_chars)
{
  size_t original_length = strlen (original);
  char *new_string = (char *)malloc (
      original_length + num_chars + 1); // Don't forget +1 for null terminator
 ...
 */
void
convertLeftToRight (const uint8_t *left_matrix, uint8_t *right_matrix)
{
 ...
 */
void
set_option (int display, uint8_t address, uint8_t value)
{
  ...
  i2c_write_timeout_us (I2C_DISPLAY_LINE, addresses[display], buf,
                        count_of (buf), false, 1000);
}
 ...
 */
uint8_t *
prepend_address (uint8_t address, uint8_t *buffer, int buffer_size)
{
...
void
set_pixel (int display, uint8_t x, uint8_t y, bool status, bool update)
{
 ...
 */
void
transform_buffer_for_display (char letter, int display, uint8_t *buffer)
{
  for (int i = 0; i < 8; i++)
    {
      if (display % 2 == 0)
 ...
 */
void
set_char (int display, char letter, bool update)
{
  if (display < 0 || display > 7)
    return;
 ...
 */
void
scroll_display_string (char *string)
{
 ...
 */
void
display_string (const char *string)
{
  clear_all (false);
...
void
animate_slide_up (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
          ...
          for (int row = 0; row < 8; row++)
            {
              ...
              displays[display][row] = (current_buffer[row] >> shift)
...
void
animate_pixelated_transition (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
...
void
animate_expanding_contracting (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
...
void
animate_morph (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
...
void
animate_interlocking_pieces (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
          ...
          if (row < 4)
            {
              ...
              displays[display][row] = (current_buffer[row] << (4 - step))
...
void
animate_glitch_effect (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
      ...
      for (int i = 0; i < 8; i++)
        {
          ...
          if (rand () % 3 == 0)
          ...
          else
            {
              ...
              glitch_buffer[i]
                  = next_buffer[i]
                    ^ (1 << (rand () % 5)); // Only flip within 5 bits
            }
...
void
animate_heartbeat_pulse (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
...
void
animate_waterfall (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
...
void
animate_clock_hand_wipe (int display, char current_char, char next_char)
{
  if (display < 0 || display > 7)
    return;
