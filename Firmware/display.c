...
void animate_slide_up(int display, char current_char, char next_char) {
  ...
  if (display % 2 == 0) {
    // Even displays: Original vertical slide becomes horizontal (right-to-left)
    ...
    // Odd displays: Original horizontal slide becomes vertical (bottom-to-top)
    ...
    for (int shift = 0; shift < 8; shift++) {
      ...
      update_display();
      sleep_us(10000);  // Adjust animation speed
    }
  }
...
void animate_pixelated_transition(int display, char current_char,
  ...
  for (int step = 0; step < 8; step++) {
    for (int row = 0; row < 8; row++) {
      temp_buffer[row] = 0;
      for (int col = 0; col < 8; col++) {
        int rand_val = rand() % (8 - step);
        if (display % 2 == 0) {
          // Even displays: Standard pixelated transition
          if (rand_val == 0) {  // Randomly choose pixels
            temp_buffer[row] |= (next_buffer[row] & (1 << col));
          } else {
            temp_buffer[row] |= (current_buffer[row] & (1 << col));
          }
        } else {
          // Odd displays: Horizontal pixelated transition
          if (rand_val == 0) {
            temp_buffer[row] |= (next_buffer[row] & (1 << (7 - col)));
          } else {
            temp_buffer[row] |= (current_buffer[row] & (1 << (7 - col)));
          }
        }
      }
...
void animate_interlocking_pieces(int display, char current_char,
  ...
  for (int step = 0; step < 4; step++) {
    for (int row = 0; row < 8; row++) {
      if (display % 2 == 0) {
        // Even displays: Top and bottom slide
        if (row < 4) {
          // Top half slides down
          displays[display][row] =
              (current_buffer[row] << (4 - step)) | (next_buffer[row] >> step);
        } else {
        ...
        // Odd displays: Left and right slide (rotated 90 degrees)
        // Treat columns as rows and rows as columns
        if (row < 4) {
          displays[display][row] =
              (current_buffer[row] >> (4 - step)) | (next_buffer[row] << step);
        } else {
    ...
    update_display();
    sleep_us(10000);  // Adjust speed as needed
  }
...
void animate_glitch_effect(int display, char current_char, char next_char) {
  ...
  for (int frame = 0; frame < 3; frame++) {
    ...
    for (int i = 0; i < 8; i++) {
      glitch_buffer[i] = 0; // Initialize glitch buffer

      for (int col = 0; col < 5; col++) { // Iterate through columns
        // Mix between current, next, and random noise
        if (rand() % 3 == 0) {
          // Random noise - only use lower 5 bits for 5×7 display
          if (display % 2 == 0) {
            glitch_buffer[i] |= (rand() % 32) & (1 << col);
          } else {
            glitch_buffer[i] |= (rand() % 32) & (1 << (4 - col));
          }
        } else if (rand() % 2) {
          // Current with horizontal shift
          int shift = (rand() % 3) - 1; // -1, 0, or 1
          uint8_t shifted = shift > 0 ? (current_buffer[i] << shift)
                                      : (current_buffer[i] >> -shift);
          if (display % 2 == 0) {
            glitch_buffer[i] |= (shifted & 0x1F) & (1 << col);
          } else {
            glitch_buffer[i] |= (shifted & 0x1F) & (1 << (4 - col));
          }
        } else {
          // Next char with some bits flipped
          uint8_t flipped = next_buffer[i] ^ (1 << (rand() % 5));
          if (display % 2 == 0) {
            glitch_buffer[i] |= (flipped & 0x1F) & (1 << col);
          } else {
            glitch_buffer[i] |= (flipped & 0x1F) & (1 << (4 - col));
          }
        }
      }

      // Sometimes swap rows (apply to both even and odd)
      if (i > 0 && rand() % 5 == 0) {
...
void animate_heartbeat_pulse(int display, char current_char, char next_char) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    temp_buffer[i] =
        current_buffer[i] & 0x15;  // Remove some pixels (binary 00010101)
  }
  ...
  for (int i = 0; i < 8; i++) {
    // For 5×7 display, only use lower 5 bits
    temp_buffer[i] =
        current_buffer[i] & 0x09;  // Remove more pixels (binary 00001001)
  }
  ...
  update_display();
  sleep_us(10000);  // Slightly longer
...
void animate_waterfall(int display, char current_char, char next_char) {
  ...
  // Get character bitmaps, considering even/odd displays
  for (int i = 0; i < 8; i++) {
    if (display % 2 == 0) {
      current_buffer[i] = characters[((int)current_char) - 32][i];
      next_buffer[i] = characters[((int)next_char) - 32][i];
    } else {
      current_buffer[i] = alternate_characters[((int)current_char) - 32][i];
      next_buffer[i] = alternate_characters[((int)next_char) - 32][i];
    }
  }
  ...
  for (int frame = 0; frame < 8; frame++) {
    for (int row = 0; row < 8; row++) {
      if (display % 2 == 0) {
        // Even displays - Left-to-right horizontal waterfall
        anim_buffer[row] = 0;
        // Add black pixel line before wipe
        if (frame > 0) {
          for (int col = 0; col < 8; col++) {
            if (col == frame - 1) {
              //  Clear this column to create a black line
        ...
        // Odd displays - Top-to-bottom vertical waterfall
        // Add black pixel line before wipe
        if (frame > 0) {
          // Clear this row to create a black line
          if (row == frame - 1) {
            anim_buffer[row] = 0x00;
          } else if (row <= frame) {
...
// Clock hand wipe - simulates analog clock hand expanding dot with controlled reveal
void animate_clock_hand_wipe(int display, char current_char, char next_char) {
  ...
  // Define center of the effect
  const float center_x = 2.0;
  const float center_y = 3.5;

  // Animation loop (radius expansion)
  for (int radius = 0; radius <= 4; radius++) {
    ...
    // (Optional) Initial Clear - very brief
    if (radius == 0) {
      for (int row = 0; row < 8; row++) {
        displays[display][row] = 0x00;
      }
      update_display();
      sleep_us(2000); // Very short clear
    }
    ...
    for (int row = 0; row < 8; row++) {
      for (int col = 0; col < 5; col++) {
        ...
        // If this pixel is within the expanding radius
        if (dist <= radius) {
          // Controlled Reveal: Prioritize next_buffer
          uint8_t bit_mask = (1 << col);
          if (next_buffer[row] & bit_mask) {
            anim_buffer[row] |= bit_mask;
          } else if (radius < 4 && current_buffer[row] & bit_mask) {
            //  Only show old character within a smaller radius
            //  This helps "mask" the old character
            if (dist <= radius -1)
            {
              anim_buffer[row] |= bit_mask;
            }
           ...
           {
             anim_buffer[row] |= next_buffer[row] & bit_mask;
           }
        }
      }
    }
    ...
    update_display();
    sleep_us(25000); // Adjust for speed as needed
  }
